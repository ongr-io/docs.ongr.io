...
for line in lines:
    if ( line[0]!="#" ) :
        ...
        os.system("cd components; rm -rf %s; mkdir %s; cd %s; git init; git remote add -f origin %s; git config core.sparsecheckout true; echo '%s' > .git/info/sparse-checkout; echo 'Resources/API' >> .git/info/sparse-checkout; git fetch -f origin master; git reset --hard;" % (repository_url.split("/")[-1], repository_url.split("/")[-1], repository_url.split("/")[-1], repository_url, doc_directory))
